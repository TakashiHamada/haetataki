...
- レベル = 撃破数 `KILLS_PER_LEVEL` 匹ごとに+1（詳細は7章。原作準拠=100、研究用=20）
...
レベルアップに必要な撃破数は定数 `KILLS_PER_LEVEL` で管理する。
原作準拠なら **100**（1面=100匹）、研究・デバッグ用途では **20** を推奨
（60匹で全敵種が出揃う）。Web版クローンは研究用に20を採用している。

| レベル | 変化 |
|---|---|
| 1 | 小型+水色のみ。基本の「叩く」と「優先順位」を学ぶ |
| 2 | 黄色が追加。「逃げる」局面が生まれる |
| 3 | 爆弾が追加。全要素が揃う |
| 4+ | 構成は固定。速度+8%/レベル（上限+60%）、同時数+1（上限7） |
